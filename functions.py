                ...
                if insert:
                    ...
                    if treeview.selection():
                        ...
                        if 'R$' in informations[2]:
                            informations = informations[0:6] + informations[10:14] + ('', '', '')
        ...
        if ask := self.message_window(4, 'Comfimação', 'Você tem certeza de que deseja deletar o(s) item(s) selecionado(s)'):
            for selection in self.selection_treeview(treeview):
                ...
                if table == 'Pedidos':
                    ...
                    itens = self.dataBases['sale'].searchDatabase(f'SELECT * FROM Itens WHERE número_do_pedido = "{selection[0]}"')
                    for item in itens:
                        amount = self.dataBases['stock'].searchDatabase(f'SELECT quantidade FROM Produtos WHERE código = "{item[5]}"')
                        self.dataBases['stock'].crud(f'UPDATE Produtos SET quantidade = "{int(amount[0][0]) + 1}" WHERE código = "{item[5]}"')

                    # deleting itens of order ===========================
                    self.dataBases[type_information].crud(f'DELETE FROM Itens WHERE número_do_pedido = "{selection[0]}"')
                    if len(self.dataBases[type_information].searchDatabase(searchAll.format('Itens'))) == 0:
                        self.dataBases[type_information].crud(f"DELETE FROM sqlite_sequence WHERE name='Itens'")
        ...
        match type_photo:
            ...
            case 'unknow':
                if photo in ['employee', 'costumer']:
                    ...
                    self.photosAndIcons[photo] = self.image(f'assets/cabide.png', (76, 76))
                label.configure(image=self.photosAndIcons[photo][0])

    def check_treeview_to_close(self, treeview, action='close'):
        treeviewsinformations = self.pick_informations_treeview(treeview)
        doAction = {
            'close': lambda: self.message_window(2, 'Pedido aberto', 'Existe um pedido aberto. Finalize o pedido ou remova os itens antes de fechar') if treeviewsinformations else [self.loginWindow.destroy(), self.backup_dataBaes_discret()],
            'blockEntry': lambda:
            [self.customSchedulingEntry.configure(state='disable', fg_color='#f4f4f4'), self.professionalSchedulingEntry.configure(state='disable', fg_color='#f4f4f4'), self.methodPaySchedulingEntry.configure(state='disable', fg_color='#f4f4f4')] if treeviewsinformations else
            [self.customSchedulingEntry.configure(state='normal', fg_color='#ffffff'), self.professionalSchedulingEntry.configure(state='normal', fg_color='#ffffff'), self.methodPaySchedulingEntry.configure(state='normal', fg_color='#ffffff')]
        }
...
class FunctionsOfSchedule(GeneralFunctions):
    ...
    def register_sale(self, informations, type_function, treeview, button, type_insert='button', entrys=None):
        # informations of treeview ====================
        match type_function:
            case 'add':
                # validating for scheduling ==================================================
                if self.validation(informations[0][0:8], 5) and self.validation(informations[0][4], 4) and self.validation(informations[0][6], 3):
                    # insert ascheduling in treeview ======================================
                    match type_insert:
                        case 'button':
                            # checking stock =========================================
                            amount = self.dataBases['stock'].searchDatabase(f'SELECT quantidade FROM Produtos WHERE código = "{informations[0][6]}"')
                            if amount and int(amount[0][0]) > 0:
                                print(informations)
                                self.insert_treeview_informations(treeview, informations, 'saleRegister')
                                # incrementing value total ======================
                                valueTotal = self.treating_numbers(type_treating=4, values=[value[4] for value in self.pick_informations_treeview(treeview)])
                                self.labelTotal.configure(text=f'Total: {valueTotal}')
                                # updating amount of item =========================
                                self.dataBases['stock'].crud(f'UPDATE Produtos SET quantidade = "{int(amount[0][0]) - 1}" WHERE código = "{informations[0][6]}"')
                            else:
                                self.message_window(2, 'Sem estoque', 'O item está em falta ou não cadastrado no estoque')
                        case 'codeEventBind':
            ...
            case 'remove':
                ...
                if treeview.selection():
                    # confirming removal ========================
                    if self.message_window(4, 'Comfimação', 'Você tem certeza de que deseja excluir o(s) item(ns)'):
                        # deleting selected information ================================
                        for selection in treeview.selection():
                            # updating amount of item =========================
                            amount = self.dataBases['stock'].searchDatabase(f'SELECT quantidade FROM Produtos WHERE código = "{treeview.item(selection, "values")[6]}"')
                            self.dataBases['stock'].crud(f'UPDATE Produtos SET quantidade = "{int(amount[0][0]) + 1}" WHERE código = "{treeview.item(selection, "values")[6]}"')
                        ...
                        self.delete_informations_treeview(treeview, 'saleRegister')
                        self.insert_treeview_informations(treeview, informationsTreeview, 'saleRegister')
                        # incrementing value total ======================
                        valueTotal = self.treating_numbers(type_treating=4, values=[value[4] for value in self.pick_informations_treeview(treeview)])
                        self.labelTotal.configure(text=f'Total: {valueTotal}')
                        # updating amount of item =========================
            ...
            case 'finishRegister':
                if informationsTreeview := self.pick_informations_treeview(treeview):
                    if self.message_window(4, 'Comfimação', 'Você tem certeza de que deseja finalizar o pedido'):
                        # calculating value total ===============================
                        valueTotal = self.treating_numbers(type_treating=4, values=[value[4] for value in informationsTreeview])
                        dateOfSale = datetime.today().strftime("%d/%m/%Y  %H:%M:%S")

                        # registing order =======================
                        self.dataBases['sale'].crud(
                            registerSale.format(
                                informationsTreeview[0][0].title(), len(informationsTreeview), valueTotal, informationsTreeview[0][7],
                                informationsTreeview[0][5].title(), dateOfSale,
                            ))

                        # searching order for register itens ==========================
                        dateOfSale = datetime.today().strftime("%d/%m/%Y  %H:%M:%S")
                        order = self.dataBases['sale'].searchDatabase(f'SELECT ID FROM Pedidos WHERE data_de_venda = "{dateOfSale}"')
                        for info in informationsTreeview:
                            self.dataBases['sale'].crud(registerItens.format(info[1], info[2], info[3], info[4], info[6], order[0][0]))

                        # cleaning order register
                        self.delete_informations_treeview(treeview, 'scheduling')
                        self.check_treeview_to_close(treeview, 'blockEntry')
                        button.invoke()
                        self.labelTotal.configure(text=f'Total:')

                        self.message_window(1, 'Concluído', messagein=f'Pedido feito com sucesso')
                else:
                    self.message_window(2, 'Sem registro', 'A tabela esta vazia')

        # updating treeview of stock ===================================================
        self.search_stock(self.treeviewStockControl, ['', '', '', '', '', '', '', '', '', '', 'ID'])
        self.check_treeview_to_close(treeview, 'blockEntry')

    def register_with_barCode(self, treeview, informations, entrys):
        # searching services with the cheat ===================================
        item = self.dataBases['stock'].searchDatabase(f'SELECT marca, modelo, peça, valor_de_venda, quantidade FROM Produtos WHERE código = "{informations[0][6]}"')
        if item:
            if int(item[0][4]) > 0:
                # inserting informations in entrys ==============================================
                for index, information in enumerate([item[0][0], item[0][1], item[0][2], item[0][3]]):
                    if isinstance(entrys[index], CTkComboBox):
                        entrys[index].set('')
                        entrys[index].set(information)
                    else:
                        ...
                        entrys[index].insert(0, information)
                # verification ===========================================
                if self.validation([informations[0][0], informations[0][5], informations[0][7]], 5) and self.validation(informations[0][4], 4) and self.validation(informations[0][6], 3):
                    # inserting services with the cheat =============================================
                    cheatInformations = [[informations[0][0], item[0][0], item[0][1], item[0][2], item[0][3], informations[0][5], informations[0][6], informations[0][7]]]
                    self.insert_treeview_informations(treeview, cheatInformations, 'saleRegister')
                    # incrementing value total ======================
                    valueTotal = self.treating_numbers(type_treating=4, values=[value[4] for value in self.pick_informations_treeview(treeview)])
                    self.labelTotal.configure(text=f'Total: {valueTotal}')
                    # updating amount of item =========================
                    self.dataBases['stock'].crud(f'UPDATE Produtos SET quantidade = "{int(item[0][4]) - 1}" WHERE código = "{informations[0][6]}"')
                ...
                self.message_window(2, 'Sem estoque', 'O item está em falta no estoque')
        else:
            self.message_window(3, 'Sem estoque', 'O item não está cadastrado no estoque')

    def search_itens(self, treeview=None, informations=None):
        # pick up informations =========================================
        order = self.dataBases['sale'].searchDatabase(f'SELECT ID FROM Pedidos WHERE data_de_venda = "{informations[5]}"')
        informationsDataBase = self.dataBases['sale'].searchDatabase(f"SELECT * FROM Itens WHERE número_do_pedido = {order[0][0]}")
    ...
    def search_sale(self, treeview=None, informations=None, type_search='new', save_seacrh=True, insert=True):
        ...
        if save_seacrh:
            self.lastSearch['sale'] = searchSale.format(
                ...
                informations[5], informations[6].replace(' ', '_').lower()
            )
        ...
        match type_search:
            case 'new':
                informationsDataBase = self.dataBases['sale'].searchDatabase(
                    searchSale.format(
                        'ID' if informations[0].isnumeric() else 'cliente', informations[0], informations[1], informations[2], informations[3], informations[4],
                        informations[5], informations[6].replace(' ', '_').lower()
                    )
            ...
            case 'last':
                informationsDataBase = self.dataBases['sale'].searchDatabase(self.lastSearch['sale'])
            case 'all':
                informationsDataBase = self.dataBases['sale'].searchDatabase(searchAll.format('Pedidos'))

        if insert:
            # deleting and inserting informations in treeview ===============================
            self.delete_informations_treeview(treeview, 'sale')
            self.insert_treeview_informations(treeview, informationsDataBase, 'sale')
            self.delete_informations_treeview(self.treeviewItens, 'itens')
        else:
            return informationsDataBase

    def update_schedule(self, treeview, informations, entrys):
        # update informations =========================================
        if self.validation([informations[0], informations[3], informations[4]], 1):
            if treeview.selection():
                informationsDataBase = self.dataBases['sale'].crud(
                    updateSale.format(
                        informations[0].title(), informations[2], informations[3], informations[4], self.selection_treeview(treeview)[0][0]
                    )
                )
                ...
                self.message_window(1, 'Concluído', messagein=f'Pedido atualizado com sucesso')
            else:
                self.message_window(3, 'Sem seleção', 'Selecione algum pedido na lista para atualizar')
        else:
            self.message_window(3, 'Erro', 'Verifique se os campos estão preenchidos')

    def delete_schedule(self, treeview):
        if treeview.selection():
            # deleting inforations =======================================
            delete = self.delete_information(treeview, 'sale', 'Pedidos')
            ...
            if delete:
                ...
                self.message_window(1, 'Concluído', messagein=f'Pedido(s) deletado(s) com sucesso')
        else:
    ...
    def create_pdf_schedule(self, treeview):
        ...
        if informationsTreeview:
            ...
            for information in informationsTreeview:
                tableWithInformationsScheduleTreeview.append(information[0:7])
        ...
        if self.message_window(4, 'Comfimação', f'Finalisar o cadastro de {informations[0].title()}?'):
            ...
            self.dataBases['informations'].crud(
                registerClient.format(
                    informations[0].title(), informations[1], informations[2], informations[3], informations[4], informations[5],
    ...
    def update_client(self, treeview, informations, entrys):
        ...
        if treeview.selection():
            informationsDataBase = self.dataBases['informations'].crud(
                updateClient.format(
                    informations[0].title(), informations[1], informations[2], informations[3], informations[4], informations[5], informations[6],
...
class FunctionsOfBarCodeInformations(GeneralFunctions):
    ...
    def register_barCode(self, informations, treeview):
        if self.validation(informations[0:1], 5) and self.validation(informations[0], 6):
            if self.message_window(4, 'Comfimação', f'Finalisar o cadastro do código?'):
                ...
                if directoryBarCode[0] != '.png':
                    # registing ====================
                    self.dataBases['informations'].crud(
                        registerBarCode.format(directoryBarCode[1], directoryBarCode[0], informations[1])
                    )
                    # deleting and inserting informations in treeview ===============================
                    self.search_barCode(treeview, informations, 'all', save_seacrh=False)
                    # refresh list =============================
                    self.refresh_combobox_barCode()
                    self.message_window(1, 'Concluído', messagein=f'O arquivo foi salvo em "{directoryBarCode[0]}"')
        else:
            self.message_window(3, 'Erro', 'Verifique se os campos estão preenchidos ou corretos')
    ...
    def search_barCode(self, treeview=None, informations=None, type_search='new', save_seacrh=True, insert=True):
        # save last search ============================================
        if save_seacrh:
            self.lastSearch['barCode'] = searchBarCode.format(informations[0], informations[1], informations[2])
        # pick up informations =========================================
        informationsDataBase = []
        match type_search:
            case 'new':
                informationsDataBase = self.dataBases['informations'].searchDatabase(searchBarCode.format(informations[0], informations[1], informations[2]))
            case 'last':
    ...
    def update_barCode(self, treeview, informations):
        if treeview.selection():
            # update informations =========================================
            informationsDataBase = self.dataBases['informations'].crud(
                updateBarCode.format(informations[1], self.selection_treeview(treeview)[0][0])
            )
    ...
    def create_image_barCode(informations):
        # creating file name ========================================
        fileName = f'Código_{informations[0].capitalize()}'
        # creating image of bar code ================================
        code = EAN13(informations[0])
        # saving image ==============================================
        ...
        if directory != '':
            ...
            cairosvg.svg2png(url=rf'{directory}/{fileName}.svg', write_to=rf'{directory}/{fileName}.png', output_width=523, output_height=280)
            os.remove(rf'{directory}/{fileName}.svg')
    ...
    def create_pdf_barCode(self, treeview):
        if informationsTreeview := self.pick_informations_treeview(treeview):
            # informations ============================================
            for information in informationsTreeview:
                tableWithInformationsBarCodeTreeview.append([information[0], information[1]])
    ...
    def refresh_combobox_barCode(self):
        # refresh list of combobox services ================================
        self.cheatSchedulingEntry.configure(values=[name[1] for name in self.search_barCode(informations=self.searching_list('', 1, 'código'), save_seacrh=False, insert=False)])
        self.barcodeSaleInventoryControlEntry.configure(values=[name[1] for name in self.search_barCode(informations=self.searching_list('', 1, 'código'), save_seacrh=False, insert=False)])
...
class FunctionsOfInformationsStock(GeneralFunctions):
    ...
    def refresh_combobox_InformationsStock(self):
        ...
        # model -------------------------------------
        self.modelSaleInventoryControlEntry.configure(values=[name[1] for name in self.search_InformationsStock(informations=self.searching_list('Modelo', 1, 'Modelo'), save_seacrh=False, insert=False)])
        # measure -------------------------------------
        self.partSaleInventoryControlEntry.configure(values=[name[1] for name in self.search_InformationsStock(informations=self.searching_list('Peça', 1, 'Peça'), save_seacrh=False, insert=False)])
    ...
    def search_init(self):
        ...
        self.search_InformationsStock(self.model['treeview'], ['Modelo', self.model['entry'].get(), self.model['order'].get()], typeInformations='model', table='Modelo')
        self.search_InformationsStock(self.part['treeview'], ['Peça', self.part['entry'].get(), self.part['order'].get()], typeInformations='part', table='Peça')


class FunctionsOfStockInformations(GeneralFunctions):

    def register_stock(self, informations, treeview, delete=False):
        # treating numbers e checking code ==========================
        valuesPrice = [self.treating_numbers(informations[5], 1), self.treating_numbers(informations[6], 1)]
        code = self.dataBases['stock'].searchDatabase(f'SELECT código FROM Produtos WHERE código = "{informations[7]}"')
        if self.validation(informations[0:8], 5) and self.validation(valuesPrice[0], 4) and self.validation(valuesPrice[1], 4) and self.validation(informations[4], 4) and self.validation(informations[7], 4) and not code:
            if self.message_window(4, 'Comfimação', f'Finalisar o cadastro do produto?'):
                self.dataBases['stock'].crud(
                    registerSaleStock.format(
                        informations[0].upper(), informations[1].upper(), informations[2].upper(), informations[3],
                        informations[4], self.treating_numbers(informations[5], 1), self.treating_numbers(informations[6], 1), informations[7], informations[8], informations[9]
                    ))
                # deleting and inserting informations in treeview ===============================
                self.search_stock(treeview, informations, 'all', save_seacrh=False)
        else:
            # show message error =====================================================
            self.message_window(3, 'Erro', 'Verifique se os campos estão preenchidos, corretos ou verifique se o código de barras já esta cadastrado')

    def search_stock(self, treeview=None, informations=None, type_search='new', save_seacrh=True, insert=True):
        # save last search ============================================
        if save_seacrh:
            self.lastSearch['product'] = searchSaleStock.format(
                informations[0], informations[1], informations[2], informations[3], informations[4],
                informations[5], informations[6], informations[7], informations[9],
                informations[10].replace('Q/', '').replace('V/', 'valor_de_').replace('D/', 'data_de_').replace(' ', '_').lower()
            )
        # pick up informations =========================================
        informationsDataBase = []
        match type_search:
            ...
            case 'last':
                informationsDataBase = self.dataBases['stock'].searchDatabase(self.lastSearch['product'])
            case 'all':
                informationsDataBase = self.dataBases['stock'].searchDatabase(searchAll.format('Produtos'))
    ...
    def update_stock(self, treeview, informations, **kwargs):
        ...
        if treeview.selection():
            # treating numbers ==========================
            valuesPrice = [self.treating_numbers(informations[5], 1), self.treating_numbers(informations[6], 1)]
            if self.validation(informations[0:8], 5) and self.validation(valuesPrice[0], 4) and self.validation(valuesPrice[1], 4) and self.validation(informations[4], 4) and self.validation(informations[7], 4):
                self.dataBases['stock'].crud(
                    updateSaleStock.format(
                        informations[0], informations[1], informations[2], informations[3],
                        informations[4], self.treating_numbers(informations[5], 1), self.treating_numbers(informations[6], 1),
                        informations[7], informations[8], informations[9], self.selection_treeview(treeview)[0][0]
                    )
                )
                # delete informations of treeview ==============================
                self.delete_informations_treeview(treeview, 'product')
    ...
    def delete_stock(self, treeview, register_in_unusable=False, treeview2=None, **kwargs):
        if treeview.selection():
            # deleting inforations =======================================
            delete = self.delete_information(treeview, 'stock', "Produtos")
    ...
    def create_pdf_stock(self, treeview):
        if informationsTreeview := self.pick_informations_treeview(treeview):
            # informations ============================================
            for information in informationsTreeview:
                tableWithInformationsSaleStockTreeview.append(information[0:9])
            tableWithInformationsComplementarySaleStock.append(self.informations_supplementarys(informationsTreeview))

            # create tables ===========================================
            table1 = Table(tableWithInformationsSaleStockTreeview)
            table1.setStyle(TableStyle(styleTableInformationsTreeview))
            table2 = Table(tableWithInformationsComplementarySaleStock)
            table2.setStyle(TableStyle(styleTableInformationsComplementary))

            # reseting tables =========================================
            del tableWithInformationsSaleStockTreeview[2:]
            del tableWithInformationsComplementarySaleStock[2:]
    ...
    def message_informations_stock(self, treeview):
        if informationsTreeview := self.pick_informations_treeview(treeview):
            ...
            self.message_window(
                1,
                'Informações sobre a tabela',
                messageSaleStock.format(*self.informations_supplementarys(informationsTreeview))
            )
        else:
            self.message_window(2, 'Sem registro', 'A tabela esta vazia')
    ...
    def informations_supplementarys(self, informations):
        ...
        generalInformations = [
            len(informations),
            max([self.treating_numbers(info=value[6], type_treating=7) for value in informations]),
            min([self.treating_numbers(info=value[6], type_treating=7) for value in informations]),
            max([self.treating_numbers(info=value[7], type_treating=7) for value in informations]),
            min([self.treating_numbers(info=value[7], type_treating=7) for value in informations])
        ]
    ...
    def refresh_combobox_stock(self):
        # refresh list of combobox informations ================================
        # brand ------------------------------------
        self.brandSchedulingEntry.configure(values=[name[2] for name in self.search_stock(informations=self.searching_list('', 9, 'Marca'), save_seacrh=False, insert=False)])
        # model -------------------------------------
        self.modelSchedulingEntry.configure(values=[name[3] for name in self.search_stock(informations=self.searching_list('', 9, 'Modelo'), save_seacrh=False, insert=False)])
        # measure -------------------------------------
        self.partSchedulingEntry.configure(values=[name[4] for name in self.search_stock(informations=self.searching_list('', 9, 'Peça'), save_seacrh=False, insert=False)])
...
class FunctionsOfCashManagement(GeneralFunctions):
    ...
    def register_cashManagement(self, informations, treeview, paremeters=None):
        ...
        if self.validation(informations[0:8], 5) and self.validation(informations[0], 3) and self.validation(informations[1:7], 8):
            if self.message_window(4, 'Comfimação', f'Registrar os dados?'):
                self.dataBases['cash'].crud(
                    paremeters['sqlRegister'].format(
                        paremeters['table'], paremeters['typeDate'], informations[0].upper(), self.treating_numbers(informations[1], 1), self.treating_numbers(informations[2], 1),
                        self.treating_numbers(informations[3], 1), self.treating_numbers(informations[4], 1), '0', '0', '0', '0',
                        self.treating_numbers(informations[5], 1), self.treating_numbers(informations[6], 1), datetime.today().strftime('%d/%m/%Y') if informations[7] == '' else informations[7].upper(),
                        informations[8], informations[11].upper()
                    ))
                # deleting and inserting informations in treeview ===============================
    ...
    def search_cashManagement(self, treeview=None, informations=None, parameters=None, type_search='new', save_seacrh=True, insert=True):
        # save last search ============================================
        if save_seacrh:
            self.lastSearch[parameters['type_cash']] = (
    ...
    def update_cashManagement(self, treeview, informations, parameters):
        ...
        if treeview.selection():
            ...
            def date(type_date):
                match type_date:
                    case 'data':
                        return datetime.today().strftime('%d/%m/%Y') if informations[7] == '' else informations[7].upper()
                    case 'mês':
                        return datetime.today().strftime('%m/%Y') if informations[7] == '' else informations[7].upper()

            if self.validation(informations[0:8], 5) and self.validation(informations[0], 3) and self.validation(informations[1:7], 8):
                # pick value for exit =======================
                exitValue = self.treating_numbers(informations[9] if informations[9] != '' else '0', 1)
                value = self.dataBases['cash'].searchDatabase(f'SELECT {"s_" + informations[10].lower() if informations[10] != "" else "s_cartão"} FROM {parameters["table"]} WHERE ID = {self.selection_treeview(treeview)[0][0]}')[0][0]
                self.dataBases['cash'].crud(
    ...
    def pick_informations_for_cash(self, entrys, date=datetime.today().strftime("%d/%m/%Y"), type_informations='day'):
        match type_informations:
            case 'day':
                ...
                informationsSale = self.dataBases['sale'].searchDatabase(f'SELECT * FROM Pedidos WHERE data_de_venda LIKE "%{date}%"')
                # Collecting informations for messege===============================
                methoPay = {
                    'card': [row for row in informationsSale if row[4] in ['CARTÃO', 'CARTAO']],
                    'money': [row for row in informationsSale if row[4] in ['DINHEIRO']],
                    'transfer': [row for row in informationsSale if row[4] in ['TRANSFERÊNCIA', 'TRANSFERENCIA']],
                    'note': [row for row in informationsSale if row[4] in ['NOTA', 'FIADO', 'NOTINHA']],
                    'notPay': [row for row in informationsSale if row[4] in ['NÃO FOI PAGO', 'SEM PAGAMENTO', 'NÃO PAGO', '']],
                }

                sumMetohdPay = [
                    len(informationsSale),
                    self.treating_numbers(type_treating=2, values=methoPay["card"]),
                    self.treating_numbers(type_treating=2, values=methoPay["money"]),
                    self.treating_numbers(type_treating=2, values=methoPay["transfer"]),
                    self.treating_numbers(type_treating=2, values=methoPay["note"]),
                    self.treating_numbers(type_treating=2, values=methoPay["notPay"]),
                    self.treating_numbers(type_treating=2, values=informationsSale)
                ]
                # inserting informations of day ====================================
                for index, information in enumerate(sumMetohdPay):
                    entrys[index].insert(0, information)
                entrys[7].insert(0, date)
                if sumMetohdPay[5] != 'R$0,00':
                    entrys[8].set('DIA EM ANDAMENTO')
                else:
                    entrys[8].set('DIA FINALIZADO')
            case 'month':
                # deleting informations of entrys ============================================
                for entry in entrys[0:9]:
                    if isinstance(entry, CTkComboBox):
                        ...
                        entry.delete(0, END)
                # informations of schedule and sold ===========================================
                informationsDay = self.dataBases['cash'].searchDatabase(f'SELECT * FROM Gerenciamento_do_dia WHERE data LIKE "%{date}%" and status = "DIA FINALIZADO"')
                ...
                for index, information in enumerate(informationsOfMonth):
                    entrys[index].insert(0, information)
                entrys[7].insert(0, date)
                if informationsOfMonth[5] not in ['R$0,00', 'R$00,00', '']:
                    entrys[8].set('MÊS EM ANDAMENTO')
                else:
                    entrys[8].set('MÊS FINALIZADO')

    def create_pdf_cashManagement(self, treeview, parameters):
        if informationsTreeview := self.pick_informations_treeview(treeview):
            # informations ============================================
            for information in informationsTreeview:
                parameters['tablePart1'].append(information[0:7])
            for information in informationsTreeview:
                parameters['tablePart2'].append(information[7:14])
    ...
    def informations_supplementarys_cashManagement(self, informations, type_date):
        ...
        return [type_date, len(informations), totalClients, totalReceived, totalExit]
...
class FunctionsOfPayment(GeneralFunctions):
    ...
    def pick_informations_for_payment(self, entrys, date, professional=''):
        # searching information of schedule =========================
        informationsSchedule = self.dataBases['sale'].searchDatabase(f'SELECT * FROM Pedidos WHERE data_de_venda LIKE "%{date}%" AND profissional LIKE "%{professional}%"')
        # treating information ===================================
        informationsPayment = [
            len([value for value in informationsSchedule if value[4] not in ['NÃO FOI PAGO', 'SEM PAGAMENTO', '']]),
            self.treating_numbers(
                type_treating=4,
                values=[value[3] for value in informationsSchedule if value[4] not in ['NÃO FOI PAGO', 'SEM PAGAMENTO', '']],
            ),
        ]
        # inserting informations in entrys ======================
        for entry in entrys[2:6]:
            entry.delete(0, END)
...
class FunctionsOfLogin(GeneralFunctions):
    ...
    def password_window(self, function, parameter):
        ...
        self.passwordWindow.title('Senha de administrador - MK Moda Feminina Pro')
